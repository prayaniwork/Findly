...
export interface WorksWheelItem {
  ...
  href?: string;
  /** Price in currency (optional metadata for commerce) */
  price?: string | number;
  /** Store or brand (optional metadata) */
  store?: string;
  /** Match percentage (optional metadata) */
  matchScore?: number;
}
...
export function WorksWheel({
  ...
  return (
    ...
    >
      ...
      <div
        ref={labelRef}
        className="pointer-events-none absolute inset-0 grid place-items-center tracking-tight font-medium"
      ...
      <div
        ref={titleRef}
        className="pointer-events-none absolute top-1/2 left-[8%] -translate-y-1/2 tracking-tight opacity-0 font-medium max-w-[40%]"
        style={{ fontSize: metrics.title }}
      >
        {items[active]?.title}
      </div>
